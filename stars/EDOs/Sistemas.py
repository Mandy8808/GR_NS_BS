...
def systemBS(r, yV, arg):
    ...
    if p0>80 or p0<-80:
        ...
        f0 = (g0 - g0**3)/(2.*r) + (2*aB*g0*r*(n0**2*p1**2 + g0**2*p0**2*(n0**2 + w**2)))/n0**2
        f1 = ((-1 + g0**2)*n0)/(2.*r) + (2*aB*r*(n0**2*p1**2 + g0**2*p0**2*(-n0**2 + w**2)))/n0
        f2 = p1
        
        #g1, n1 = f0, f1
        f3 = -(p1/r) + g0**2*(-(p1/r) + p0*(1 + 4*aB*p0*p1*r - w**2/n0**2))
        #f3 = (g1*p1)/g0 - (p1*(2*n0 + n1*r))/(n0*r) + g0**2*p0*(1 - w**2/n0**2)
    else:
        f0 = 0
        f1 = 0
        f2 = p1
