...
def solver(I, xmax, tmax, dt, C, m=0, t0=0, x0=0, c=1, NoHomog=None, f=None, V=None, user_action=None, Vp=lambda u: 0,
    ...
    para una malla (xmin, xmax) \times (tmin, tmax).
    ...
    utilez = [dt, f, V, C, m]
    u = zerostep(xi, ti, u_n, utilez, uL=uL, uR=uR, Vp=Vp)
    ...
    for n in range(1, Nt):
        utilez = [dt, f, C, m]
        u = nsteps(n, xi, ti, u_n, u_nm1, utilez, uL=uL, uR=uR, Vp=Vp)
...
# Iteraciones #
###############
def zerostep(xi, ti, u_n, utilez, uL=None, uR=None, Vp=lambda u: 0):
    """
    ...
    Nx = u_n.size - 1
    u = np.zeros(Nx+1)
    u[1:-1] = u_n[1:-1] + dt*Vi + 0.5*C2*(u_n[2:] - 2*u_n[1:-1] +\
              u_n[:-2]) + 0.5*dt**2*(fi - m*u_n[1:-1] - Vp(u_n[1:-1]))
...
def nsteps(n, xi, ti, u_n, u_nm1, utilez, uL=None, uR=None, Vp=lambda u: 0):
    """
    Primera iteración temporal
    ...
    Nx = u_n.size
    u = np.zeros(Nx)
    u[1:-1] = -u_nm1[1:-1] + 2*u_n[1:-1] + C2*(u_n[2:] - 2*u_n[1:-1]\
             +u_n[:-2]) + dt**2*(fi - m*u_n[1:-1] - Vp(u_n[1:-1]))
